...
export default function HamburgerMenu({ categories }: { categories: { name: string; href: string }[] }) {
        ...
        <ul className="p-4 mt-[20px]">
          {Object.entries(menuItems).map(([name, href]) => (
            <li key={name} className="relative py-2 text-black">
              {/* Special handling for Magazin and Cursuri Profesionale */}
              ...
              {/* Dropdown for Magazin */}
              {name === "MAGAZIN" && dropdownOpen && (
                <ul className="pl-4">
                  {/* Link pentru toate produsele */}
                  <li className="py-2">
                    <Link
                      href="/magazin"
              ...
              {/* Dropdown for Cursuri Profesionale */}
              {name === "CURSURI PROFESIONALE" && cursuriDropdownOpen && (
                <ul className="pl-4">
                  {(href as any[]).map((course) => (
